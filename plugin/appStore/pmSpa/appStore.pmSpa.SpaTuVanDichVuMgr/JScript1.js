var SpaTuVanDichVuMgrFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=appStore.pmSpa.SpaTuVanDichVuMgr.Class1, appStore.pmSpa.SpaTuVanDichVuMgr'; },
    setup: function () { },
    loadgrid: function () {
        adm.styleButton();
        adm.loading('Đang lấy dữ liệu');
        var KH_ID = $('.filter-tuVan-byKhachHang');
        $('#SpaTuVanDichVuMgrMdl-List').jqGrid({
            url: SpaTuVanDichVuMgrFn.urlDefault().toString() + '&subAct=get&',
            height: '1000',
            datatype: 'json',
            loadui: 'disable',
            ...
            sortname: 'NgayTao',
            sortorder: 'desc',
            pager: jQuery('#SpaTuVanDichVuMgrMdl-Pager'),
            rowNum: 10,
    ...
    refresh: function (grid) {
        if (typeof (grid) == 'undefined') grid == '#SpaTuVanDichVuMgrMdl-List';
        var KH_ID = $('.filter-tuVan-byKhachHang');
        KH_ID.val('');
        KH_ID.attr('_value', '');
        var timerSearch;
        ...
        timerSearch = setTimeout(function () {
            $(grid).jqGrid('setGridParam', { url: SpaTuVanDichVuMgrFn.urlDefault().toString() + '&subAct=get' }).trigger('reloadGrid');
        }, 500);
    },
    edit: function (grid) {
        var s = '';
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                SpaTuVanDichVuMgrFn.loadHtml(function () {
                    var newDlg = $('#SpaTuVanDichVuMgrMdl-dlgNew');
                    $(newDlg).dialog({
                        title: 'Sửa',
                        width: 1000,
                        buttons: {
                            'Lưu': function () {
                                SpaTuVanDichVuMgrFn.save(false, function () {
                                    SpaTuVanDichVuMgrFn.clearform();
                                }, grid);
                            },
                            'Lưu và đóng': function () {
                                SpaTuVanDichVuMgrFn.save(false, function () {
                                    SpaTuVanDichVuMgrFn.clearform();
                                    $(newDlg).dialog('close');
                                    SpaTuVanDichVuMgrFn.draffTuVan(function (_item) { });
                                }, grid);
                            },
                            'Đóng': function () {
                                $(newDlg).dialog('close');
                        ...
                        open: function () {
                            ...
                            adm.styleButton();
                            SpaTuVanDichVuMgrFn.clearform();
                            SpaTuVanDichVuMgrFn.popfn();
                            $.ajax({
                                url: SpaTuVanDichVuMgrFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': s
                                },
                                success: function (_dt) {
                                    ...
                                    DV_DanhSach.find('.tt-item-xoaBtn').unbind('click').click(function () {
                                        ...
                                        $.ajax({
                                            url: SpaTuVanDichVuMgrFn.urlDefault().toString(),
                                            type: 'POST',
                                            dataType: 'script',
                                            data: {
                                                'subAct': 'delTinhTrang',
    ...
    add: function (fn) {
        SpaTuVanDichVuMgrFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        SpaTuVanDichVuMgrFn.save(false, function (_ID, _Ten) {
                            if (typeof (fn) == 'function') {
                                fn(_ID, _Ten);
                            }
                            SpaTuVanDichVuMgrFn.clearform();
                            SpaTuVanDichVuMgrFn.draffTuVan(function (_item) { });
                        }, '#SpaTuVanDichVuMgrMdl-List');
                    },
                    'Lưu và đóng': function () {
                        SpaTuVanDichVuMgrFn.save(false, function (_ID, _Ten) {
                            if (typeof (fn) == 'function') {
    ...
    del: function (grid) {
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                if (confirm('Bạn có chắc chắn xóa tư vấn này?')) {// Xác nhận xem có xóa hay không
                    $.ajax({
                        url: SpaTuVanDichVuMgrFn.urlDefault().toString() + '&subAct=del',
                        dataType: 'script',
                        data: { 'ID': s },
                        success: function (dt) {
    ...
    search: function () {
        ...
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $('#SpaTuVanDichVuMgrMdl-List').jqGrid('setGridParam', { url: SpaTuVanDichVuMgrFn.urlDefault().toString() + '&subAct=get&KH_ID=' + _KH_ID}).trigger('reloadGrid');
        }, 500);
    },
    popfn: function () {
        var newDlg = $('#SpaTuVanDichVuMgrMdl-dlgNew');
    ...
    autoCompleteSearch: function (el, fn, fn1, fn2) {
        $(el).autocomplete({
            source: function (request, response) {
                var term = 'SpaTuVanDichVuMgr-autoCompleteSearch' + request.term;
    ...
    loadHtml: function (fn) {
        var dlg = $('#SpaTuVanDichVuMgrMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("appStore.pmSpa.SpaTuVanDichVuMgr.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
